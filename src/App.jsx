...
const socket = io("https://bingo-server-bvpo.onrender.com", { transports: ["websocket"] });
const SIZE = 5;

const emptyGrid = () => Array.from({ length: SIZE }, () => Array(SIZE).fill(null));
const emptyMarked = () => Array.from({ length: SIZE }, () => Array(SIZE).fill(false));

const randomGrid = () => {
  const nums = Array.from({ length: SIZE * SIZE }, (_, i) => i + 1).sort(() => Math.random() - 0.5);
  let k = 0;
  return Array.from({ length: SIZE }, () => Array.from({ length: SIZE }, () => nums[k++]));
};
...
export default function App() {
  ...
  useEffect(() => {
    ...
    socket.on("init", ({ board }) => setBoard(board));
  ...
  const manualFill = (i, j) => {
    ...
    if (nextNum === SIZE * SIZE) setFilled(true);
    setNextNum((n) => n + 1);
  };
  ...
  const startGame = () => socket.emit("ready");
  ...
  return (
    <div className="app">
      <h1>🎯 Online Bingo</h1>
      <p>Last Number: <b>{calledNumber ?? "-"}</b></p>
        ...
        {board.map((row, i) =>
          row.map((cell, j) => (
            <button
              ...
              onClick={() => (started ? callNumber(cell) : manualFill(i, j))}
            >
              {cell ?? ""}
            </button>
          ))
      ...
      {started && !winner && <p className="turn">{myTurn ? "🟢 Your Turn" : "⏳ Opponent Turn"}</p>}
    </div>
  );
}
